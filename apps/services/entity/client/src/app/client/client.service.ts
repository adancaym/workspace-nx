...
import { ClientServiceContract } from '@workspace-nx/contracts';
import { CreateClient, ReadClient, UpdateClient } from '@workspace-nx/swagger';
...
export class ClientService implements ClientServiceContract {
  ...
  create(createClientDto: CreateClient): Promise<ReadClient> {
    return this.repository
      .save(this.repository.create(createClientDto))
      .then((e) => new ReadClient(e));
  }

  findAll(): Promise<ReadClient[]> {
    return this.repository.find().then((e) => e.map((e) => new ReadClient(e)));
  }
