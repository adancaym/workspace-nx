...
export class CustomerClientService implements CustomerClientServiceContract {
  ...
  create(
    createCustomerClientDto: CreateCustomerClient
  ): Promise<ReadCustomerClient> {
    return this.repository
      .save(createCustomerClientDto)
  ...
  findAll(): Promise<ReadCustomerClient[]> {
    return this.repository
      .find()
      .then((e) => e.map((e) => new ReadCustomerClient(e)));
  }
  ...
  update(
    id: number,
    updateCustomerClientDto: UpdateCustomerClient
  ): Promise<ReadCustomerClient> {
    return this.repository
      .update(id, updateCustomerClientDto)
