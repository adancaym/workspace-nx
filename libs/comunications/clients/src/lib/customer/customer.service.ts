...
import { ICreateCustomer, ICustomer, IUpdateCustomer } from '@workspace-nx/models';
...
export class CustomerService {
    ...
    async create(customer: ICreateCustomer): Promise<ICustomer> {
        const customerCreated = await this.proxy.send('createCustomer', customer).toPromise();
    ...
    async update(id: number, customer: IUpdateCustomer): Promise<ICustomer> {
        const customerUpdated = await this.proxy.send('updateCustomer', { ...customer, id }).toPromise();
        return customerUpdated;
    }

    async remove(id: number) {
        const customerDeleted = await this.proxy.send('removeCustomer', id).toPromise();
